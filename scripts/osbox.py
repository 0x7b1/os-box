...
if sys.version_info[0] < 3:
    ...
    print('You have invoked OSBox with Python 2. OSBox must be run with Python 3.')
    sys.exit()
...
class Zenity:
    ...
    def password():
        ...
        while True:
            ...
            getPasswordCmd = runCmd(['zenity', '--password', '--title=OSBox', '--window-icon=alfred.png'])
...
class Alfred:
    ...
    def __init__(self, localRecipes=True):
        ...
        if not supportedDistro:
            message = "This is not an Ubuntu or Ubuntu derivative distro. You can't run OSBox on this system."
        ...
        if arch.stdout != 'x86_64\n':
            message = "This is not a 64-bit system. You can't run OSBox on this system."
        ...
        if lock.stdout != '':
            message = 'Another program is installing or updating packages. Please wait until this process finishes and then launch OSBox again.'
        ...
        if not ping.succeeded:
            message = 'There is no connection to the Internet. Please connect and then launch OSBox again.'
    ...
    def showPackages(self):
        while True:
            ...
            if table.stdout == '':
                Zenity.info('No package was selected')
        ...
        if checkLock:
            ...
            if not waitForDpkgLock(): # Wait for /var/lib/dpkg/lock to be released
                ...
                with open(self.logFile, 'a') as f:
                    ...
                    Zenity.error('Another program is installing or updating packages. Please wait until this process finishes and then launch OSBox again.')
                    sys.exit()
